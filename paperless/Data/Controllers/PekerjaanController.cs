...
namespace paperless.Data.Controllers
{
    ...
    public class PekerjaanController : Controller
    {
        ...
        [Authorize]
        [HttpPost]
        public IActionResult LoadDetailpekerjaan ([FromBody] DetailPekerjaan Peku)
        {
            JObject jReturn = new JObject();
            var statusCode = 200;
            List<dynamic> retData = new List<dynamic>();
            String mpi_idpekerjaan = Convert.ToString(Peku.Eid.ToString());

            try
            {
                retData = lp.LoadDetailpekerjaan (mpi_idpekerjaan);
                if (retData.Count > 0)
                {
                    jReturn.Add("status", mc.GetMessage("api_output_ok"));
                    jReturn.Add("code", statusCode);
        ...
        [Authorize]
        [HttpPost]
        public IActionResult UpdateLogsheet([FromBody] Formlogsheet1 Peku)
        {
            JObject jReturn = new JObject();
            var statusCode = 200;
            List<dynamic> retData = new List<dynamic>();
            ...
            try
            {
                ...
                string status = lp.UpdateFormlogsheet(Peku);
                if (status == "success")
                {
                    jReturn.Add("status", mc.GetMessage("api_output_ok"));
                    jReturn.Add("code", statusCode);
        ...
        [Authorize]
        [HttpPost]
        public IActionResult Deletehistory ([FromBody] DeletePekerjaan dpk)
        {
            JObject jReturn = new JObject();
            var statusCode = 200;
            List<dynamic> retData = new List<dynamic>();
            ...
            try
            {
                retData = lp.Deletehistory(mpi_idpekerjaan);
                if (retData.Count > 0)
                {
                    jReturn.Add("status", mc.GetMessage("api_output_ok"));
                    jReturn.Add("code", statusCode);
                    jReturn.Add("message", mc.GetMessage("Delete_success"));
                }
                else
                {
                    statusCode = 404;
        ...
        [Authorize]
        [HttpPost]
        public IActionResult Listpekerjaantrx([FromBody] FilterTrx fil)
        {
            JObject jReturn = new JObject();
            var statusCode = 200;
            List<dynamic> retData = new List<dynamic>();
            String mpi_idpekerjaan1 = Convert.ToString(fil.Tanggalaw.ToString());
            String mpi_idpekerjaan2 = Convert.ToString(fil.Tanggalak.ToString());
            String mpi_idpekerjaan3 = Convert.ToString(fil.PekDescr.ToString());

            try
            {
                retData = lp.LoadPekerjaantrx (mpi_idpekerjaan1, mpi_idpekerjaan2, mpi_idpekerjaan3);
                if (retData.Count > 0)
                {
                    jReturn.Add("status", mc.GetMessage("api_output_ok"));
                    jReturn.Add("code", statusCode);
        ...
        [Authorize]
        [HttpPost]
        public IActionResult ListLokasi([FromBody] Lokasi pek)
        {
            JObject jReturn = new JObject();
            var statusCode = 200;
            List<dynamic> retData = new List<dynamic>();
            ...
            try
            {
                retData = lp.ReadLokasi (mpi_idpekerjaan);
                if (retData.Count > 0)
                {
                    jReturn.Add("status", mc.GetMessage("api_output_ok"));
                    jReturn.Add("code", statusCode);
